...
import { getSupplierStatementData } from "@/lib/bills-service";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function SupplierStatementDialog({
  open,
  onOpenChange,
  supplierId,
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getSupplierStatementData(supplierId, startDate, endDate),
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              <div className="bg-muted p-4 rounded-md">
                ...
                <p className="text-sm text-muted-foreground">{data.supplier?.email}</p>
              </div>
