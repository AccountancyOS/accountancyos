    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="space-y-4">
          <div><Label>PAYE Scheme</Label>
            <Select value={schemeId} onValueChange={setSchemeId}>
              ...
              <SelectContent>{schemes?.map(s => <SelectItem key={s.id} value={s.id}>{s.employer_paye_reference} - {s.companies?.company_name || 'Individual'}</SelectItem>)}</SelectContent>
            </Select>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => createMutation.mutate()} disabled={!firstName || !lastName || !schemeId || createMutation.isPending}>Add</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
