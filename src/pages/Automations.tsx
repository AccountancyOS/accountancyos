    ...
    queryFn: async () => {
      ...
      for (const exec of data || []) {
        if (!byRule[exec.automation_rule_id]) {
          byRule[exec.automation_rule_id] = { status: exec.status, executed_at: exec.executed_at || "" };
        }
      }
      return byRule;
    },
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Manage workflow automations and custom rules
            </p>
          </div>
                          ...
                          {filteredRules.map((rule) => {
                            ...
                            return (
                              <TableRow key={rule.id}>
                                ...
                                <TableCell>
                                  <RequirePermission permission="can_manage_automation_rules">
                                    <Switch checked={rule.is_active} onCheckedChange={(checked) => toggleMutation.mutate({ id: rule.id, is_active: checked })} />
                                  </RequirePermission>
                                </TableCell>
                                <TableCell>
                                  <RequirePermission permission="can_manage_automation_rules">
                                    <DropdownMenu>
                                      ...
                                      <DropdownMenuContent align="end">
                                        <DropdownMenuItem onClick={() => { setEditingRule(rule); setEditorOpen(true); }}>
