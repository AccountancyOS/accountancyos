...
serve(async (req) => {
  ...
  try {
    ...
    if (authError || !user) {
      console.error("[RTI Submit] Auth error:", authError);
      return new Response(
    ...
    console.log(`[RTI Submit] Processing ${messageType} submission for filing ${filingId} by user ${user.id}`);
    ...
    if (orgError || !orgUser) {
      console.error(`[RTI Submit] User ${user.id} not authorized for org ${organizationId}`);
      return new Response(
    ...
    if (filingError || !filing) {
      console.error(`[RTI Submit] Filing ${filingId} not found or not in org ${organizationId}`);
      return new Response(
