-- Consolidation P1 — route public_accept_quote_by_token through lifecycle_materialize_jobs
...
BEGIN
  ...
  IF v_quote.lead_id IS NOT NULL THEN
    UPDATE public.leads SET status = 'won', updated_at = now() WHERE id = v_quote.lead_id;
  END IF;
