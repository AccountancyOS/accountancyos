-- =====================================================================================
-- Proposal Phase 2 T2d-1 — ONE implementation of the engagement-letter signature rule.
-- =====================================================================================
-- NO BEHAVIOUR CHANGE. This increment only removes a duplicate-logic risk and adds the
-- vocabulary the per-signatory signing flow (T2d-2) will write.
--
-- Why:
--   T2b put the signature RULE ('all' = every required signatory signed / 'any' = at
--   least one) INLINE inside lifecycle_onboarding_gates. T2d-2 introduces the WRITER
--   side (the per-signatory sign RPC), which must evaluate exactly the same rule. Two
--   copies of that rule in two functions is precisely the drift this project has been
--   bitten by before, so the rule becomes a single callable function FIRST, and the
--   gate is re-pointed at it in the same migration.
--
-- What this migration does:
--   (A) NEW  public.el_signature_progress(uuid) -> jsonb  — the rule, once.
--   (B) RE-ISSUE public.lifecycle_onboarding_gates(uuid) — byte-identical to the LIVE
--       body (def-hash 6f60145c17a3f46b894ef8807005012a, which is the body this repo
--       shipped in 20260727130000_split_onboarding_gate.sql) EXCEPT that the inline
--       signature-rule block now calls (A). Semantically equivalent — see the
--       equivalence note at the call site.
--   (C) EXTEND engagement_letters_status_check with 'partially_signed'.
--       Widening only: every value previously allowed is still allowed, so no existing
--       row can be invalidated. T2d-2 is what starts writing the new value.
--
-- Not touched: lifecycle_approve_onboarding, lifecycle_evaluate_onboarding_activation,
-- any signing RPC, any RLS policy, any table data.
-- =====================================================================================

-- (A) The signature rule, as one function ----------------------------------------------
-- Returns the full progress picture rather than a bare boolean, because both callers
-- need more than "satisfied": the gate needs satisfied + whether signatory rows exist at
-- all (to decide on the legacy single-signer fallback), and the T2d-2 sign RPC needs the
-- signed/total counts to decide between 'partially_signed' and 'signed'.
--
--   { has_signatories: bool,   -- false => caller applies its own legacy fallback
--     signing_rule:    text,   -- 'all' | 'any' (defaults to 'all')
--     required_total:  int,
--     required_signed: int,
--     satisfied:       bool }  -- always false when has_signatories is false
--
-- STABLE + SECURITY DEFINER so the (STABLE, SECURITY DEFINER) gate can call it and so
-- the public token-based sign path can reach signatory rows that RLS hides from anon.
...
-- (B) Re-issue the gate so it calls (A) --------------------------------------------------
...
-- (C) engagement_letters.status gains 'partially_signed' ---------------------------------
-- The signatory rows stay the source of truth for the rule (see (A)); this column is the
-- human/UI-facing projection of it: draft -> sent -> partially_signed -> signed.
--
-- The CHECK is located by the column it constrains, not by name, so a live constraint
-- carrying a different name is replaced rather than silently left behind to reject the
-- new value. Widening only — no currently-stored value is removed.
DO $$
DECLARE
  v_attnum smallint;
  v_con    record;
