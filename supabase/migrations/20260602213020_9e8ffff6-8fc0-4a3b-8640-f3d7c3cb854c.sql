-- 1. Remove duplicate uuid overload to eliminate ambiguity
DROP FUNCTION IF EXISTS public.public_accept_quote_by_token(uuid);

-- 2. Replace the text-overload acceptance function with full job + deadline provisioning
...
BEGIN
  ...
  -- Load company record once for period derivation
  IF v_company_id IS NOT NULL THEN
    SELECT * INTO v_company FROM public.companies WHERE id = v_company_id;
  END IF;

  -- Compute UK tax year (6 Apr -> 5 Apr) covering today
  ...
  -- Create engagements + jobs + deadlines per line
  FOR v_line IN
  ...
  LOOP
    ...
    -- Idempotency: skip if engagement already exists for this quote+service
    ...
    -- Derive period based on entity + service
    ...
    IF v_target_company IS NOT NULL AND v_company.year_end_month IS NOT NULL AND v_company.year_end_day IS NOT NULL THEN
      -- Build the most recent year-end on or before today
    ...
    -- Compute filing deadline per service code
    ...
    -- Create job (idempotent on engagement)
    ...
    -- Create deadlines (statutory only, with valid filing_body)
    IF v_filing_deadline IS NOT NULL AND v_line.code IN ('company_accounts','confirmation_statement','corporation_tax','sa_mtd','sa_non_mtd','vat_return','payroll') THEN
      ...
      -- CT payment deadline (separate from filing)
