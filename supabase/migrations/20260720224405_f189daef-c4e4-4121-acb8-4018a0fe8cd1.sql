...
DROP FUNCTION IF EXISTS public.set_signatory(uuid, boolean);
DROP FUNCTION IF EXISTS public.link_person_to_sa_client(uuid, uuid);
DROP FUNCTION IF EXISTS public.grant_person_portal_access(uuid, text);
...
BEGIN
  ...
  SELECT EXISTS (SELECT 1 FROM public.company_officers WHERE company_id = p_company_id AND person_id = p_person_id)
      OR EXISTS (SELECT 1 FROM public.contacts WHERE company_id = p_company_id AND person_id = p_person_id)
    INTO v_associated;
...
BEGIN
  ...
  FROM public.company_officers o JOIN public.companies co ON co.id = o.company_id
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_organization_id uuid; v_linked_client_id uuid; v_target_user_id uuid;
...
BEGIN
  ...
  IF v_linked_client_id IS NOT NULL THEN
    ...
    IF v_target_user_id IS NOT NULL THEN
      SELECT EXISTS (SELECT 1 FROM public.portal_access WHERE client_id = v_linked_client_id AND user_id = v_target_user_id AND is_active AND status <> 'revoked') INTO v_already_active;
    END IF;
  ...
  FOR v_company IN SELECT DISTINCT company_id FROM public.company_officers WHERE person_id = p_person_id AND resigned_at IS NULL
  LOOP
    v_already_active := false;
    IF v_target_user_id IS NOT NULL THEN
      SELECT EXISTS (SELECT 1 FROM public.portal_access WHERE company_id = v_company.company_id AND user_id = v_target_user_id AND is_active AND status <> 'revoked') INTO v_already_active;
    END IF;
    IF v_already_active THEN v_skipped := v_skipped + 1;
