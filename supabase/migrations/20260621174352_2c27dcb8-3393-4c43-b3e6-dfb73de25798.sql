-- ============================================================
-- Sprint 1 — Increment 2 / Task A
-- public_accept_quote_by_token -> pending funnel (flag-gated)
-- ============================================================
-- When the org's canonical_lifecycle_enabled flag is ON, quote acceptance no
-- longer creates ACTIVE rows (accountant_client_links / engagements / jobs /
-- deadlines). It still creates the pending client/company, marks the quote
-- accepted and the lead won. The client then completes onboarding and is
-- activated via the gated approval. Flag OFF => byte-identical to 20260603200107
-- (only the v_canonical declaration/assignment and IF NOT v_canonical guards
-- are added; verified by diff). resolve_company_director is reproduced verbatim.
-- ============================================================


-- Helper: resolve a director's identity for a company so that SA / individual-scoped
-- work can be assigned to the right person instead of the company name.
...
BEGIN
  ...
  -- 1. contacts table for the company
  IF p_company_id IS NOT NULL THEN
  ...
  -- 2. company_officers + company_persons
  IF p_company_id IS NOT NULL THEN
  ...
  -- 3. ch_company_profile -> officers JSON (CH format: "LAST, First Middle")
  IF p_company_id IS NOT NULL THEN
  ...
  -- 4. lead first/last, only if not equal to company name
...
-- Update quote acceptance: SA client must be in the director's name, not the company's.
...
BEGIN
  ...
  -- Company must be resolved first so director lookup can use it.
  ...
  IF v_has_individual AND v_client_id IS NULL AND v_lead.id IS NOT NULL THEN
    -- If the quote also has a company, the SA client is the director, not the lead/company.
    IF v_has_company THEN
      ...
      ELSE
        -- No director on file: skip SA client creation and flag for the practice.
        v_sa_blocked := true;
