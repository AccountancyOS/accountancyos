-- ============================================================
-- Invoicing Stage 5 — let portal clients create/edit/issue invoices (permissioned)
-- ============================================================
-- The invoice RPCs were org-member-only, so portal clients could never raise or send an
-- invoice. Extend ONLY the auth checks: create/edit require portal 'allow_invoice_create';
-- issue (which posts to AR + is used by Send) requires portal 'allow_invoice_send' AND that
-- the practice has NOT set require_review_for_invoice_sending (server-enforced sign-off).
-- Accountant (org member) paths are unchanged. Bodies reproduced byte-faithfully.
-- ============================================================
