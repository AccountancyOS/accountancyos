...
BEGIN
  ...
  SELECT count(*) INTO v_setup_pending
    FROM public.jobs WHERE organization_id = _org_id AND status = 'Setup Pending';
  ...
  SELECT COALESCE(count(*), 0), COALESCE(sum(n - 1), 0)
    INTO v_dup_groups, v_dup_excess FROM dups;
  ...
  SELECT COALESCE(array_agg(indexname), ARRAY[]::text[])
    INTO v_present
    FROM pg_indexes
    WHERE schemaname = 'public' AND indexname = ANY(v_expected_idx);
