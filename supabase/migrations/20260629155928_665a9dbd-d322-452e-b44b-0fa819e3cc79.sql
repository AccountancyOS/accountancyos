-- ============================================================
-- Consolidation P1 — route public_accept_quote_by_token through the canonical engine
-- ============================================================
-- Replaces accept's inline engagement/job/deadline creation (the exact logic the
-- engine was extracted from) with one lifecycle_materialize_jobs call. Job/deadline
-- creation now has a SINGLE implementation shared by accept + approve (+ manual/rollover
-- next). Behaviour-preserving; idempotent. Rest of the function byte-faithful from
-- 20260625070935. The accountant_client_links IF-NOT-canonical guards are untouched.
-- ============================================================
...
BEGIN
  ...
  IF NOT v_canonical THEN
    -- Single canonical engine: engagements + jobs + deadlines now live in
    -- lifecycle_materialize_jobs (shared by approval / manual Add Job / rollover).
    -- This replaces the former inline copy; behaviour unchanged. Idempotent.
