-- ============================================================
-- Forward-only DIVERGENCE REPAIR — lifecycle_approve_onboarding
-- ============================================================
-- Increment 1 (Seam 2: onboarding-created jobs get statutory deadlines).
--
-- Problem (verified live 2026-07-24 via catalog_functions, live def hash
-- 5a7c9aa0c10beddb9a80394a2590e2a5): the LIVE lifecycle_approve_onboarding runs an
-- OLD inline job-creation loop that inserts jobs with automation_source='template',
-- NO period_start/period_end, NO job_template_code, and never calls the canonical
-- engine. Result: onboarding-approved jobs get ZERO statutory deadlines (neither the
-- canonical trigger nor lifecycle_upsert_job_with_deadlines ever runs for them).
--
-- Root cause: live-vs-git divergence. git-HEAD already fixed this in migration
-- 20260629091343 (route through lifecycle_materialize_jobs — the confirmed SINGLE
-- SOURCE of truth for engagements+jobs+deadlines: every other live path — quote
-- acceptance, add-service, rollover, chaser-trigger-scan — uses it, and
-- src/test/regression/job-creation-single-source.test.ts enforces it). That fix
-- never took effect on live; live drifted back to the pre-consolidation body.
--
-- Repair: re-issue the canonical definition forward-only (per the migration release
-- contract, historical files are never re-run). Verified byte-for-byte that the live
-- body and git-HEAD 20260629091343 differ ONLY in the `IF v_onboarding.quote_id IS
-- NOT NULL` block; every other statement is identical, so no live-only content is
-- lost. lifecycle_materialize_jobs is idempotent and dedupes, so approval will not
-- double-create against anything quote-acceptance already made.
--
-- Post-apply verification (definition-exact, per release convention):
--   1. catalog_functions name_like 'lifecycle_approve_onboarding' — body MUST contain
--      "PERFORM public.lifecycle_materialize_jobs(" and MUST NOT contain
--      "'onboarding_approval:' || p_onboarding_id::text)\n        )\n        RETURNING id INTO v_job_id" (the inline INSERT).
--   2. Behavioural: approve a real onboarding for an entity whose company has
--      year_end_month/day (or an SA/individual), then confirm rows appear in
--      public.deadlines for the created jobs with non-null due_date.
-- NB: pre-existing onboarding jobs with automation_source='template' + filing_deadline
-- NULL are legacy/test data and are handled separately (cleanup or backfill), not here.
-- ============================================================
...
BEGIN
  ...
  IF v_onboarding.quote_id IS NOT NULL THEN
    ...
    -- Dedupes against rows quote-acceptance already created, so approval never produces a
    -- second (crude, no-deadline) job. Replaces approve's old inline creation. See
    -- lifecycle_materialize_jobs / docs/consolidation-audit-and-plan.md.
    ...
    LOOP
      -- Resolve the engagement + job the engine created (for return arrays + records-request).
