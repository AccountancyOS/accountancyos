...
DECLARE
  ...
  v_missing text[];
BEGIN
  -- Membership check: caller must belong to the org (or be service_role)
  ...
  SELECT count(*) INTO v_jobs_total
    FROM public.jobs WHERE organization_id = _org_id;

  SELECT count(*) INTO v_setup_pending
    FROM public.jobs
    WHERE organization_id = _org_id AND status = 'Setup Pending';

  SELECT count(*) INTO v_null_period
    FROM public.jobs
  ...
  -- Duplicate groups (same org + entity + service + period_label, non-null label)
  WITH dups AS (
    SELECT count(*) AS n
    FROM public.jobs
    WHERE organization_id = _org_id
      AND period_label IS NOT NULL AND period_label <> ''
    GROUP BY organization_id, COALESCE(client_id::text, ''), COALESCE(company_id::text, ''),
             COALESCE(service_id::text, ''), period_label
    HAVING count(*) > 1
  )
  SELECT COALESCE(count(*), 0), COALESCE(sum(n - 1), 0)
    INTO v_dup_groups, v_dup_excess
    FROM dups;
  ...
  WITH ndups AS (
    ...
    FROM public.jobs
    WHERE organization_id = _org_id
      AND (period_label IS NULL OR period_label = '')
    GROUP BY organization_id, COALESCE(client_id::text, ''), COALESCE(company_id::text, ''),
  ...
  SELECT array_agg(indexname)
    ...
    WHERE schemaname = 'public'
      AND indexname = ANY(v_expected_idx);

  v_present := COALESCE(v_present, ARRAY[]::text[]);
  SELECT array_agg(x) INTO v_missing
    FROM unnest(v_expected_idx) x
    WHERE x <> ALL(v_present);
...
GRANT EXECUTE ON FUNCTION public.lifecycle_reconciliation_report(uuid) TO authenticated, service_role;
