-- ============================================================
-- Consolidation P1 — route lifecycle_approve_onboarding through the canonical engine
-- ============================================================
-- Replaces approve's inline engagement + crude (calendar-year, no-deadline) job
-- creation with one call to lifecycle_materialize_jobs (idempotent). This KILLS the
-- duplicate-job bug: approval no longer makes a second job; the engine dedupes against
-- whatever quote-acceptance already created and produces the proper tax-year job +
-- deadlines. Records-request email/task logic is preserved (re-resolving the engine's
-- job). Rest of the function reproduced byte-for-byte from 20260624080129.
-- ============================================================
