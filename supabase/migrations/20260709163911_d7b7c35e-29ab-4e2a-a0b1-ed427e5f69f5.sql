...
DECLARE
  ...
  v_active_links int;
  v_accepted_no_onboarding int;
  v_onboarding_unlinked int;
  v_dup_onboarding_groups int;
...
BEGIN
  ...
  -- Inc 8.3: accept→approve handoff integrity
  SELECT count(*) INTO v_accepted_no_onboarding
    FROM public.quotes q
    WHERE q.organization_id = _org_id
      AND q.status = 'accepted'
      AND NOT EXISTS (
        SELECT 1 FROM public.onboarding_applications oa
        WHERE oa.quote_id = q.id AND oa.organization_id = _org_id
      );

  SELECT count(*) INTO v_onboarding_unlinked
    FROM public.onboarding_applications oa
    WHERE oa.organization_id = _org_id
      AND oa.status = 'approved'
      AND oa.client_id IS NULL
      AND oa.company_id IS NULL;

  WITH oadups AS (
    SELECT count(*) AS n FROM public.onboarding_applications
    WHERE organization_id = _org_id AND quote_id IS NOT NULL
    GROUP BY quote_id
    HAVING count(*) > 1
  )
  SELECT COALESCE(count(*),0) INTO v_dup_onboarding_groups FROM oadups;
  ...
  RETURN jsonb_build_object(
    ...
    'accepted_quotes_without_onboarding', v_accepted_no_onboarding,
    'onboarding_apps_unlinked', v_onboarding_unlinked,
    'duplicate_onboarding_app_groups', v_dup_onboarding_groups,
