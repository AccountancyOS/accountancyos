-- Extend email_queue.context allowed values to include 'quote' and 'engagement'
...
ALTER TABLE public.email_queue ADD CONSTRAINT email_queue_context_check
  CHECK (context = ANY (ARRAY['invoice'::text, 'chase'::text, 'onboarding'::text, 'filing'::text, 'ad-hoc'::text, 'portal'::text, 'system'::text, 'quote'::text, 'engagement'::text]));

-- Re-create lifecycle_send_quote to tag context as 'quote' instead of 'ad-hoc'
