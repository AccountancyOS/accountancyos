-- ============================================================
-- Sprint 1 — Increment 2 / Task B
-- Gated approval ensures the active accountant_client_link (canonical mode)
-- ============================================================
-- Reproduces lifecycle_approve_onboarding verbatim from 20260621160701 and adds
-- ONE block before the final status='approved' UPDATE: when the org's canonical
-- flag is ON, ensure the active practice<->entity link (the one activation
-- output the approve body does not otherwise create — accept stops creating it
-- in canonical mode, Increment 2 Task A). Idempotent (lookup-before-insert +
-- acl_active_client_uq / acl_active_company_uq). Flag OFF => block skipped =>
-- byte-identical to the current definition (verified by diff).
-- ============================================================
...
BEGIN
  ...
  -- (everything below this guard is byte-identical to 20260603180743).
