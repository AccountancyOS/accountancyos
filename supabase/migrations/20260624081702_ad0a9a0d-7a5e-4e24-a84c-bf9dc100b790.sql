-- ============================================================
-- Security fix: org-access check on lifecycle_evaluate_onboarding_activation
-- ============================================================
-- The evaluator (granted to `authenticated`) had no authorization check, so an
-- authenticated user could call it on ANY application id: with the flag ON it
-- could flip another org's application to 'for_review' (and inject review_feedback)
-- or create a spurious active accountant_client_link; with the flag OFF it could
-- read any application's gate status. Add user_has_organization_access immediately
-- after loading the application, before any read of gates / flag / write — mirroring
-- lifecycle_approve_onboarding. Body otherwise reproduced verbatim from
-- 20260624075723 (CREATE OR REPLACE, same signature).
--
-- NOTE: this makes the evaluator an ACCOUNTANT-context tool. When auto-activation
-- (triggered from the client's onboarding-completion / a system context, which has
-- no org membership) is built, it must use a SEPARATE function that is NOT granted
-- to anon/authenticated and whose authorization is the passing gates — not a bypass
-- flag on this function. See docs/sprint1-completion-runbook.md.
-- ============================================================
...
BEGIN
  ...
  -- Authorization: only members of the application's organization may evaluate or
  -- activate it. Gates every path below (read + write). Mirrors lifecycle_approve_onboarding.
