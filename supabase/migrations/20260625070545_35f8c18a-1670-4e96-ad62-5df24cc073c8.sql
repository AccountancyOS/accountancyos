-- ============================================================
-- Fix: SA (no-company) quote acceptance threw
--      'record "v_company" is not assigned yet'
-- ============================================================
-- public_accept_quote_by_token only assigned v_company when the quote had a
-- company. For a self-assessment quote (no company) v_company stayed unassigned,
-- and the legacy-path guard `IF v_target_company IS NOT NULL AND
-- v_company.year_end_month IS NOT NULL ...` threw, because PL/pgSQL binds every
-- operand of the condition before the AND can short-circuit. Initialise
-- v_company with NULL fields on the no-company branch. Body reproduced verbatim
-- from 20260624080017 with ONLY this block changed (the only v_company fields
-- referenced are year_end_month/day, both covered). Diagnosis credit: Lovable.
-- ============================================================
...
BEGIN
  ...
  ELSE
    -- Self-assessment / no-company quotes: initialise v_company so the year-end
    -- guard below can read v_company.year_end_month/day without throwing
    -- 'record "v_company" is not assigned yet' (PL/pgSQL binds every operand
    -- of the IF before the boolean AND short-circuits).
