...
ALTER TABLE public.invoice_payments ADD COLUMN IF NOT EXISTS exchange_rate numeric(18,8) NOT NULL DEFAULT 1.0;
ALTER TABLE public.bill_payments ADD COLUMN IF NOT EXISTS exchange_rate numeric(18,8) NOT NULL DEFAULT 1.0;

-- =====================================================================
-- record_invoice_payment with FX gain/loss handling
-- =====================================================================
...
BEGIN
  ...
  IF p_bank_account_id IS NOT NULL THEN
    ...
    -- because we are pre-converting). post_to_ledger uses p_fx_rate as a divisor.
    v_bank_base    := ROUND(p_amount / NULLIF(v_payment_rate,0), 2);
    v_debtors_base := ROUND(p_amount / NULLIF(v_invoice_rate,0), 2);
...
-- =====================================================================
-- record_bill_payment with FX gain/loss handling
-- =====================================================================
...
BEGIN
  ...
  IF p_bank_account_id IS NOT NULL THEN
    v_bank_base      := ROUND(p_amount / NULLIF(v_payment_rate,0), 2);
    v_creditors_base := ROUND(p_amount / NULLIF(v_bill_rate,0), 2);
