...
-- Safety net: backfill organization_id from parent quote if missing
CREATE OR REPLACE FUNCTION public.quote_acceptance_tokens_backfill_org()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  IF NEW.organization_id IS NULL AND NEW.quote_id IS NOT NULL THEN
    SELECT organization_id INTO NEW.organization_id FROM public.quotes WHERE id = NEW.quote_id;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_quote_acceptance_tokens_backfill_org ON public.quote_acceptance_tokens;
CREATE TRIGGER trg_quote_acceptance_tokens_backfill_org
BEFORE INSERT ON public.quote_acceptance_tokens
FOR EACH ROW EXECUTE FUNCTION public.quote_acceptance_tokens_backfill_org();
