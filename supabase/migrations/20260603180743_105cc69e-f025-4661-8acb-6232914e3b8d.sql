-- Fix 1: pgcrypto resolution. generate_invite_token() runs with search_path=public,
-- but gen_random_bytes lives in the `extensions` schema on Supabase. Add extensions
-- to search_path (and apply the same hardening to the wrapper) so portal invites work.
...
ALTER FUNCTION public.lifecycle_grant_portal_access(text, uuid, text, text)
  SET search_path TO 'public', 'extensions';

-- Fix 2: Surface portal-grant failures instead of swallowing them silently.
-- The function still catches the error so approval succeeds, but we now record
-- the SQLSTATE and re-emit the error in the JSON result so the UI can warn.
-- (Body otherwise unchanged; we redeploy the whole function to update the EXCEPTION block.)
