...
-- =============================================================================
-- PHASE 4 SLICE 2: Atomic apply_bank_rule hardening
-- Adds locked-period check, scope/ownership validation, accountant-only check,
-- and bookkeeping_audit_log entries.
-- =============================================================================
...
BEGIN
  ...
  -- Caller must be an organisation member (accountant). Portal users are blocked.
  ...
  -- Load rule and validate organisation + entity scope
  ...
  END IF;
  -- Scope: if the rule is bound to a client/company it must match the txn entity
  ...
  -- Load bank GL account + validate ownership/scope
  SELECT * INTO v_bank FROM bank_accounts WHERE id = v_txn.bank_account_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error_code', 'bank_account_not_found',
                              'error_message', 'Bank account not found');
  END IF;
  IF v_bank.organization_id <> v_txn.organization_id THEN
    RETURN jsonb_build_object('success', false, 'error_code', 'bank_account_org_mismatch',
                              'error_message', 'Bank account does not belong to this organisation');
  END IF;
  ...
  INSERT INTO bookkeeping_audit_log(
    ...
    jsonb_build_object('post_result', v_post_result)
  );
