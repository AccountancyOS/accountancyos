...
ALTER TABLE public.email_queue ALTER COLUMN scheduled_at SET DEFAULT (now() + interval '15 minutes');
...
BEGIN
  ...
  UPDATE public.email_queue
     SET scheduled_at = now(),
         status = CASE WHEN status IN ('pending','queued','failed','draft') THEN 'pending' ELSE status END,
         error_message = NULL,
         updated_at = now()
   WHERE id = p_email_id;
...
GRANT EXECUTE ON FUNCTION public.send_queued_email_now(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.flush_email_queue_now(uuid) TO authenticated;